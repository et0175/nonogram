...
**Closed:** 2026-08-27T18:00:54Z
**Actual:** 0.1d
**Merge commit:** aed158c
**Blocked by:** —
...
is external by construction. **Note the tension for the reviewer: the card's G-3 says
"the solver consumes the clue API", and at the module level ADR-0007 forbids exactly
that** — flagging this explicitly rather than silently picking a side.
...
[Scope] Predicted Touches matched, plus 3 docstring-only test-package __init__.py files (tests/, tests/helpers/, tests/property/) — a new-files-near-scope addition (SCOPE DISCIPLINE explicitly allows this), needed so tests.helpers.brute_force_oracle imports by name without a sys.path prelude or conftest.py. pyproject.toml untouched (G-4).
[Build gate] PASSED (full, independently re-run by orchestrator: 534 passed, 0 failed, 3.7s). EC-001 property test independently timed: 1.31s for 2400 cases (test_never_false_positive_uniqueness), well within the ≥1000-case checkpoint.
[Review 1/3] Score: 9.0 — crit: 0, imp: 1. STRUCTURE-5 tension resolved: reviewer ruled this is acceptable engineering judgment, NOT a decomposition defect — ADR-0007's Decision text explicitly names clues.py as a capability module (not a shared kernel), the CARD-001 layering guard mechanically enforces this with a merged, disk-walking test that would fail red on the import, and the card's "consumes the clue API" phrase is satisfied at the contract level (solve() takes/returns CARD-002's boundary types) without needing the module import. The self-check losing independence is a bounded, correctly-priced downgrade to a secondary signal — ADR-0014 already establishes the oracle, not the self-check, as CON-005's real evidence. Reviewer independently corroborated EC-001 with a THIRD from-scratch oracle (shares no code with either the solver or the two in-repo oracles) over 3000 cases: 0 mismatches. Fail-fast (AC-017) independently confirmed across 22 orders of magnitude of solution-count growth (24x24 with 6.2e23 solutions returns MANY in 22ms). Guardrails G-1..G-5 all ✓ holds. One Important finding: (I-1) propagate.py:238's docstring still claims "search still verifies finished grids through clues.compute_clues" — false; STRUCTURE-5's whole point is that it verifies natively via mask_runs instead. A stale statement about exactly the decision under review, on the function that decision produced — one-line fix. Reviewer's own prose recommends merging with this as a trivial follow-up, but per the severity gate an Important finding routes to a fix cycle regardless of score/reviewer framing. Also noted (Minor, non-gating): M-1 solver/oracle disagree on untested 0-cell degenerate grids; M-2 both in-repo oracles route through clues.encode_line for ground truth (closed by the reviewer's third independent oracle, above); M-3 solution distinctness argument is sound but uncommented; M-4/M-5 test-design nits. → routed to fix cycle.
[Fix 1] I-1 resolved: propagate.py's mask_runs docstring now accurately states it's what verifies finished grids (search._verified_grid calls it natively, never clues.compute_clues) and explains why (ADR-0007 forbids the lateral import; independence bought back from the test tree instead). M-3 resolved: added a comment on the disjointness invariant (sibling search branches are disjoint by construction, so 2 recorded solutions are always distinct). M-1 resolved conservatively: added a directed test pinning solution_count=1 for 0-cell grids as correct solver behavior (documented why), left the oracle's differing convention alone since EC-001's 1x1..8x8 corpus never reaches the degenerate case. 535/535 passed (534 + 1 new directed test), independently re-verified by orchestrator.
[Review 2/3] Score: 9.4 ✓ — crit: 0, imp: 0. All three findings (I-1, M-1, M-3) independently re-verified resolved with code-checked evidence, not just re-stated: reviewer traced _verified_grid's actual mask_runs calls, confirmed no nonogram.clues import anywhere in solver/, and checked the distinctness argument's load-bearing premise (Board.assign raises on an already-decided cell; every propagation write is monotone) rather than just accepting the comment. Guardrails G-1..G-5 spot-checked clean (delta is comment/docstring/test-only). 535/535, 0 regressions. Final verdict: PASS — ready to merge.
