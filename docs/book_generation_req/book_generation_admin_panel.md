...
- **BK-8 Answer key.** Answers show the filled grid only (no clues), in puzzle-number order, each captioned "Puzzle N — Title" (the only place the picture's title appears). A page holds up to **6** answers (2 × 3) while every answer on it is ≤20 on the longest side, and becomes a **4**-answer page (2 × 2) as soon as it would hold one bigger than 20. Each level (Easy / Medium / Hard) starts a new answer page with a small heading; a "SOLUTIONS" divider page precedes the answer key; the caption title is the per-book title set in Arrangement, else the puzzle's name; answer cells are capped at **5 mm** so small answers don't balloon (a 10×10 would otherwise get ~7.9 mm). That keeps every answer cell at about 3 mm or more (20 cells in a 6-up tile → 3.7 mm; 30 cells in a 4-up tile → 3.1 mm). For the default plan (90 puzzles ≤20, 60 bigger) the answer key is about 30 pages instead of 150 at one per page.
- **BK-9 Interior and cover are separate files.** KDP takes the interior PDF without the cover. The book export produces an **interior PDF** that starts at the guide page — interior page 1 is a right-hand page, which fixes the mirrored-margin parity — and the **cover separately**. Sections (level dividers, SOLUTIONS) simply flow on — no blank pages are inserted to force them onto a right-hand page. (A full KDP cover wrap with spine and back cover is a later step.)
...
| BK-9 | The cover is page 1 of the one book PDF | Interior PDF without the cover; cover exported separately |
